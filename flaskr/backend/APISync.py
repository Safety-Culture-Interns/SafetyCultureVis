...
class API:
    ...
    def write_audits_to_db(self, audit_ids):
        ...
        while (len(audit_ids) - count) != 0:
            if (len(audit_ids) - count) >= 10:
                ...
                count += 10
                self.batch_write_to_db(ten_ids)
    ...
    def batch_write_to_db(self, ids):
        batch = {
            'requests': [
        ...
        request = requests.post(url="{}/{}".format(self.url, 'batch'), headers=self.header, data=batch)
        data = request.json()
        self.MongoDB.write_many_to_mongodb(data)
